...
Even when both are enabled, write access applies only to the **server lane**: workspace backend code (e.g. an Express server) reads its per-workspace bridge credential from `RAGTIME_BRIDGE_TOKEN_FILE` and calls the runtime bridge. The **browser lane** — `context.components[componentId].execute()` in previews and shared previews — is always read-only, because its request payload is client-supplied and there is no server-side binding of a component ID to a registered query. Granting it write access would let anyone with a share URL execute arbitrary mutations with `curl`.
