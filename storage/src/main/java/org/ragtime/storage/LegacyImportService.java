...
final class LegacyImportService {
    ...
    LegacyImportService(Registry registry, StorageEngine engine) {
        this.registry = registry;
        this.importer = new LegacyImporter(registry, engine);
    }
    ...
    private void failOrCancel(String workspaceId) {
        ...
        try {
            registry.mutate(state -> {
                ObjectNode current = job(state, workspaceId);
                ObjectNode workspace = workspace(state, workspaceId);
                if (current == null || !active(current)) return;
                if (workspace == null || "revoked".equals(workspace.path("state").asText())) {
                    current.put("state", "cancelled").put("error", "workspace revoked");
                    return;
                }
                current.put("state", "failed").put("error", "staged legacy import failed");
                workspace.put("state", "importing").put("legacy_import_state", "failed");
            });
        } finally {
    ...
    private void transition(String workspaceId, String state, boolean complete, LegacyImporter.Result result) {
        ...
        registry.workspaceLock(workspaceId).writeLock().lock();
        try {
            registry.mutate(snapshot -> {
                ObjectNode current = job(snapshot, workspaceId);
                ObjectNode workspace = workspace(snapshot, workspaceId);
                if (current == null || workspace == null || "revoked".equals(workspace.path("state").asText())
                        || !active(current) || !current.path("generation").asText().matches("[0-9a-f]{32}")
                        || !current.path("manifest_sha256").asText().matches("[0-9a-f]{64}")) {
                    throw new IllegalStateException("legacy import is no longer eligible");
                }
                if (complete) {
                    current.put("state", "completed").put("verification_sha256", result.evidenceSha256());
                    workspace.put("state", "ready").put("legacy_import_state", "completed");
                    return;
                }
                current.put("state", state);
                workspace.put("state", "importing").put("legacy_import_state", state);
            });
        } finally {
            registry.workspaceLock(workspaceId).writeLock().unlock();
            registry.maintenanceLock().readLock().unlock();
    ...
    static boolean active(JsonNode job) {
        String state = job.path("state").asText();
        return state.equals("pending") || state.equals("copying");
    }

    static ObjectNode job(ObjectNode state, String workspaceId) {
        JsonNode value = state.path("legacy_import_jobs").get(workspaceId);
        return value instanceof ObjectNode job ? job : null;
    }

    private ObjectNode job(String workspaceId) {
        return job(registry.snapshot(), workspaceId);
    }

    static ObjectNode workspace(ObjectNode state, String id) {
        JsonNode value = state.path("workspaces").get(id);
        return value instanceof ObjectNode workspace ? workspace : null;
    }
}
