...
class EmbeddingModel(BaseModel):
    ...
    context_limit: Optional[int] = None
    loaded: Optional[bool] = None
    loaded_instances: Optional[List[dict[str, Any]]] = None
...
async def fetch_embedding_models(request: EmbeddingModelsRequest, _user: User = Depends(require_admin)):
    ...
    return EmbeddingModelsResponse(
        success=False,
        message=f"Unknown or unsupported embedding provider: {request.provider}. Supported: {', '.join(EMBEDDING_PROVIDER_NAMES)}",
    )


def _to_embedding_model(model: Any) -> EmbeddingModel:
    """Map provider-specific model metadata to the shared API model shape."""
    return EmbeddingModel(
        ...
        loaded_instances=getattr(model, "loaded_instances", None),
...
async def _fetch_local_embedding_models(
    provider: str,
    base_url: str,
    *,
    selected_model: str = "",
    ...
    try:
        ...
        if normalized_provider == "llama_cpp":
            ...
            return EmbeddingModelsResponse(
                success=False,
                message=f"Unsupported local embedding provider: {provider}",
            )

        models = [_to_embedding_model(model) for model in discovered]
        return EmbeddingModelsResponse(
            success=True,
            message=f"Found {len(models)} {label} embedding model(s).",
            models=models,
        ...
        return EmbeddingModelsResponse(
            success=False,
            message=f"Failed to fetch {label} embedding models: {str(e)}",
        )


async def _fetch_lmstudio_embedding_models(
    base_url: str,
    *,
    selected_model: str = "",
) -> EmbeddingModelsResponse:
...
def _resolve_ollama_chat_base_url(settings: AppSettings) -> str:
    ...
    return resolve_provider_base_url(settings, "ollama", "llm")
...
def _resolve_llama_cpp_chat_base_url(settings: AppSettings) -> str:
    ...
    return resolve_provider_base_url(settings, "llama_cpp", "llm")


def _resolve_lmstudio_chat_base_url(settings: AppSettings) -> str:
    """Resolve the effective LM Studio base URL used for chat model discovery."""
...
async def _fetch_llm_models_for_provider(
    provider: str,
    *,
    settings: AppSettings,
    api_key: str = "",
    ...
    if normalized_provider == "ollama":
        ...
        if not resolved_base_url:
            ...
            return LLMModelsResponse(success=False, message="Ollama is not configured")
...
async def _safe_fetch_llm_models_task(
    provider: str,
    fetch: Awaitable[LLMModelsResponse],
    *,
    ...
    try:
        ...
        return (
            ...
            LLMModelsResponse(
                success=False,
                message=f"Failed to fetch {label} models: {str(e)}",
            ),
        )
...
async def _fetch_lmstudio_llm_models(base_url: str, api_key: str | None = None) -> LLMModelsResponse:
    ...
    try:
        ...
        return LLMModelsResponse(
            success=True,
            message=f"Found {len(models)} LM Studio model(s).",
            models=models,
            default_model=models[0].id if models else None,
        )
    except Exception as e:
        return LLMModelsResponse(
            success=False,
            message=f"Failed to fetch LM Studio models: {str(e)}",
        )
...
class LmStudioModelLoadRequest(BaseModel):
    ...
    base_url: str = Field(default="", description="LM Studio base URL")
    model: str = Field(..., description="LM Studio model key to load")
    context_length: Optional[int] = Field(
        default=None,
        ge=1,
        description="Optional context length to use when loading the model",
    )


class LmStudioModelUnloadRequest(BaseModel):
    """Request to unload an LM Studio model instance."""

    base_url: str = Field(default="", description="LM Studio base URL")
    instance_id: str = Field(default="", description="Loaded LM Studio instance id")
    model: str = Field(
        default="",
        description="Optional model key; the first loaded instance is used when instance_id is omitted",
    )


class LmStudioModelActionResponse(BaseModel):
    """Response for LM Studio load/unload actions."""
...
def _extract_lmstudio_instance_id(instance: dict[str, Any]) -> str:
    for key in ("instance_id", "identifier", "id", "model_identifier"):
...
@router.post(
    ...
    response_model=LmStudioModelActionResponse,
    tags=["Settings"],
)
async def load_lmstudio_model(
    request: LmStudioModelLoadRequest,
    _user: User = Depends(require_admin),
):
    ...
    try:
        ...
        return LmStudioModelActionResponse(
            success=True,
            message=f"LM Studio model '{request.model}' load requested.",
            data=data,
        )
    except Exception as e:
        return LmStudioModelActionResponse(
            success=False,
            message=f"Failed to load LM Studio model: {str(e)}",
        )
    ...
    if app_settings.openai_api_key and len(app_settings.openai_api_key) > 10:
        tasks.append(
            asyncio.create_task(
                _safe_fetch_llm_models_task(
                    "openai",
                    _fetch_openai_models(app_settings.openai_api_key),
                    none_on_error=True,
                )
            )
        )
    ...
    if ollama_url:
        tasks.append(
            asyncio.create_task(
                _safe_fetch_llm_models_task(
                    "ollama",
                    _fetch_ollama_llm_models(ollama_url),
                    none_on_error=True,
                )
            )
        )
    ...
    if llama_cpp_url:
        tasks.append(
            asyncio.create_task(
                _safe_fetch_llm_models_task(
                    "llama_cpp",
                    _fetch_llama_cpp_llm_models(llama_cpp_url),
                    none_on_error=True,
                )
            )
        )
    ...
    if github_auth_error:
        ...
        tasks.append(
            asyncio.create_task(
                _safe_fetch_llm_models_task(
                    "github_copilot",
                    _fetch_github_models_catalog(github_models_token),
                    none_on_error=True,
                )
            )
        )
...
async def _to_shared_conversation_response(
    ...
    owner_username, owner_display_name = normalize_user_identity(
        str(getattr(owner_user, "username", "") or ""),
        str(getattr(owner_user, "displayName", "") or ""),
...
async def _resolve_selected_tool_ids_for_request(
    ...
    workspace_id: Optional[str],
    required_role: str,
) -> tuple[Optional[str], set[str], Optional[dict[str, Any]]]:
...
async def _build_workspaces_conversation_state_summary(
    ...
    return [
        WorkspaceConversationStateSummaryItem(
            workspace_id=workspace_id,
            has_live_task=workspace_id in live_workspace_ids,
            has_interrupted_task=workspace_id in interrupted_workspace_ids,
        )
...
@router.get("/conversations/{conversation_id}/share-links/events")
async def stream_conversation_share_link_events(
    conversation_id: str,
    request: Request,
    user: User = Depends(get_current_user),
):
        ...
        while True:
            ...
            try:
                if not first_iteration:
                    payload = await userspace_service.list_conversation_share_links(
                        conversation_id,
                        user.id,
                        is_admin=(user.role == "admin"),
                    )
