...
- Use them to avoid browser/CORS/cookie/binary parsing friction. Surface primitive error `detail` values to users. Apps still own upload UI, validation, persistence schema, domain logic, and may ignore these primitives or build their own routes/parsers/storage. Use `user_fingerprint` instead of raw user IDs in app audit logs, and never ask users for platform credentials inside the userspace app.
