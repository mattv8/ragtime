...
_USERSPACE_AUTH_DISCOVERY_NAMED_GUIDANCE = "First call `discover_userspace_primitives`;"
_USERSPACE_AUTH_DISCOVERY_GENERIC_GUIDANCE = "First use the available primitives-discovery guidance;"
_USERSPACE_IDENTITY_ENTITLEMENT_CONFIG_NAMED_GUIDANCE = "configure policy with `configure_userspace_identity_entitlements`"
_USERSPACE_IDENTITY_ENTITLEMENT_CONFIG_GENERIC_GUIDANCE = "configure policy with the available identity-entitlement configuration tool"
...
def _build_userspace_mode_prompt_template(available_tool_names: set[str] | None) -> str:
    ...
    prompt_template = prompt_template.replace(
        _USERSPACE_AUTH_DISCOVERY_NAMED_GUIDANCE,
        _USERSPACE_AUTH_DISCOVERY_NAMED_GUIDANCE
        if _userspace_tool_name_visible(available_tool_names, "discover_userspace_primitives")
        else _USERSPACE_AUTH_DISCOVERY_GENERIC_GUIDANCE,
    )
    prompt_template = prompt_template.replace(
        _USERSPACE_IDENTITY_ENTITLEMENT_CONFIG_NAMED_GUIDANCE,
        _USERSPACE_IDENTITY_ENTITLEMENT_CONFIG_NAMED_GUIDANCE
        if _userspace_tool_name_visible(available_tool_names, "discover_userspace_primitives")
        or _userspace_tool_name_visible(available_tool_names, "configure_userspace_identity_entitlements")
        else _USERSPACE_IDENTITY_ENTITLEMENT_CONFIG_GENERIC_GUIDANCE,
    )
    prompt_template = prompt_template.replace(
...
- Preview apps may use same-origin `/__ragtime/*` primitives when useful; they are optional conveniences, not app architecture. Use `discover_userspace_primitives` for compact primitive discovery. In app code, call relative same-origin paths like `fetch("/__ragtime/session")`; do not browse, curl, or guess `localhost:$PORT/__ragtime/*`. Available: `GET /__ragtime/capabilities`, `GET /__ragtime/session` (capabilities, auth methods, `auth.user` display identity, `auth.interactive_auth_endpoint`, and workspace-scoped `user_fingerprint` for audit trails), `GET|POST /__ragtime/browser-auth/start` (platform-owned interactive sign-in/SSO handoff; navigate the top-level window here with `surfaces`, optional `auth_method_key`, and `return_to` query params), `POST /__ragtime/browser-auth` (request scoped browser authorization only after the browser is already authenticated), `POST /__ragtime/parse-document`, `POST /__ragtime/parse-spreadsheet`, `POST /__ragtime/tables/normalize`, `POST /__ragtime/documents/render-preview`, `POST /__ragtime/archives/extract`, `POST /__ragtime/upload-target`, `GET|PUT /__ragtime/files/{{path}}`, `GET|PUT /__ragtime/objects/{{bucket}}/{{key}}`, `GET|PUT /__ragtime/progress/{{taskId}}`, and `POST|GET|PUT /__ragtime/jobs`.
- Use them to avoid browser/CORS/cookie/binary parsing friction. Surface primitive error `detail` values to users. Apps still own upload UI, validation, persistence schema, domain logic, and may ignore non-auth primitives or build their own routes/parsers/storage.
- For requests to add authentication, login, access control, SSO, LDAP, OAuth, Basic Auth, or audit identity, default to the platform auth primitives. First call `discover_userspace_primitives`; when it exposes `identity_entitlements`, choose auth groups by stable ID from the returned catalog, configure policy with `configure_userspace_identity_entitlements` before writing authorization code, and stop with a clear report when a required group is missing because a Ragtime administrator must sync or register it. Read the private entitlement header only in backend code, validate entitlements on every protected request, use auth.entitlements only for UI hints, and preserve the verified SSO username plus `user_fingerprint` in audit records. If app content must be protected, it is mandatory to add `<meta name="ragtime-auth" content="required; surfaces=collab,runtime_pty">` to the initial HTML document `<head>` before app scripts. Edit the real initial HTML source (`index.html`, server template, or root HTML response); adding the tag dynamically from JS/TS after load is too late and does not protect first paint. The preview proxy enforces this declaration before sending app HTML, preventing protected-content flash before login. When that declaration is present, the proxy injects `window.__ragtime_session` before app scripts and the bridge exposes the same data as `context.session`/`context.auth`; use that synchronous state for first render instead of waiting on an async `/__ragtime/session` fetch. To initiate login, redirect or link the top-level browser window to `auth.interactive_auth_endpoint` with query params such as `surfaces=collab&surfaces=runtime_pty&auth_method_key=ldap&return_to=/`; the platform owns the sign-in UI, LDAP/internal/local provider handling, cookie minting, and redirect back to the app. Use `POST /__ragtime/browser-auth` only after `auth.authenticated` is true or when handling the endpoint's `authentication_required.interactive_auth_url` response. Do not duplicate Ragtime's sign-in screen inside the app. Do not implement custom username/password prompts, localStorage login gates, hardcoded credentials, or HTTP Basic Auth emulation inside the userspace app unless the user explicitly asks for a separate app-owned auth system after you explain the platform primitive option.
- For authenticated user UI, use `auth.user.display_name` as the best user-facing label, with `auth.user.username` only as a fallback. Do not derive visible names or initials from raw user IDs or `user_fingerprint`.
- Use `user_fingerprint` instead of raw user IDs in app audit logs, and never ask users for platform credentials inside the userspace app.
