...
- Preview apps may use same-origin `/__ragtime/*` primitives when useful; they are optional conveniences, not app architecture. Available: `GET /__ragtime/capabilities`, `GET /__ragtime/session` (capabilities, auth methods, and workspace-scoped `user_fingerprint` for audit trails), `POST /__ragtime/browser-auth` (request scoped browser authorization with `auth_method_key` and `surfaces`), `POST /__ragtime/parse-document`, `POST /__ragtime/parse-spreadsheet`, `POST /__ragtime/tables/normalize`, `POST /__ragtime/documents/render-preview`, `POST /__ragtime/archives/extract`, `POST /__ragtime/upload-target`, `GET|PUT /__ragtime/files/{{path}}`, `GET|PUT /__ragtime/objects/{{bucket}}/{{key}}`, `GET|PUT /__ragtime/progress/{{taskId}}`, and `POST|GET|PUT /__ragtime/jobs`.
