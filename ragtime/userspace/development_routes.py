...
async def get_bootstrap(workspace_id: str, request: Request, principal: DevelopmentPrincipal = Depends(resolve_development_principal)) -> dict[str, Any]:
    ...
    try:
        await authorize_external_content(
            {"workspace_id": workspace_id, "operation": "bootstrap"},
            direction="inbound",
            principal=principal,
            surface="development",
        ...
        result = build_bootstrap_manifest(origin=str(request.base_url).rstrip("/"), workspace_id=workspace_id, scopes=principal.scopes)
        await authorize_external_content(
            result,
            direction="outbound",
            principal=principal,
...
async def get_bootstrap_file(
    ...
    try:
        await authorize_external_content(
            {"workspace_id": workspace_id, "artifact_id": artifact_id, "revision": revision},
            direction="inbound",
            principal=principal,
            surface="development",
            resource_id=workspace_id,
            operation="bootstrap_file",
        )
        await development_service._workspace(principal, workspace_id, "read")
