...
async def list_sqlite_history(workspace_id: str, database_name: str | None = Query(default=None), snapshot_id: str | None = Query(default=None), user: Any = Depends(get_current_user)):
    await _manage(workspace_id, user)
    service = get_sqlite_history_service()
    return {"workspace_id": workspace_id, "backups": await service.list_backups(workspace_id, database_name=database_name, snapshot_id=snapshot_id), "can_manage": True, "interrupted_maintenance": await service.interrupted_maintenance(workspace_id)}
...
async def capture_sqlite_history(workspace_id: str, request: SqliteHistoryCaptureRequest, user: Any = Depends(get_current_user)):
    ...
    if backup is None: raise HTTPException(status_code=404, detail="Database not found")
...
async def delete_sqlite_history(workspace_id: str, backup_id: str, user: Any = Depends(get_current_user)):
    await _manage(workspace_id, user); await get_sqlite_history_service().delete(workspace_id, backup_id)
    return {"success": True}
...
async def preview_sqlite_history(workspace_id: str, backup_id: str, request: SqliteHistoryPreviewRequest, user: Any = Depends(get_current_user)):
    await _manage(workspace_id, user)
    return await get_sqlite_history_service().preview(workspace_id, backup_id, mode=request.mode, conflict_policy=request.conflict_policy, table_policies=request.table_policies, user_id=user.id)
