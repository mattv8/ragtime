...
async def request_app_restart(workspace_id: str, request: RequestAppRestart, user: Any = Depends(get_current_user)):
    return await _runtime_service().request_app_restart(workspace_id, user.id, request.idempotency_key, request.reason)


@router.get("/runtime/workspaces/{workspace_id}/app/operations/{operation_id}", response_model=UserSpaceRuntimeOperation)
async def get_app_runtime_operation(workspace_id: str, operation_id: str, user: Any = Depends(get_current_user)):
    return await _runtime_service().get_app_runtime_operation(workspace_id, user.id, operation_id)
