...
def _names(database_names: list[str] | set[str] | tuple[str, ...] | None) -> list[str]:
    if database_names is None:
        return []
    # Import at the call boundary to avoid the queue/history import cycle.
...
class SqliteBackupQueueStore:
    ...
    async def enqueue(
        ...
        *,
        ...
        request_hash = hashlib.sha256(json.dumps({"trigger": trigger, "database_names": names, "snapshot_id": snapshot_id, "snapshot_git_commit_hash": snapshot_git_commit_hash, "requested_by_id": requested_by_id}, sort_keys=True).encode()).hexdigest()
        db = await get_db()
        async with db.tx() as tx:
            await tx.query_raw("SELECT 1 AS locked FROM pg_advisory_xact_lock(hashtextextended($1, 0))", _QUEUE_LOCK)
            existing = await tx.query_raw(f"SELECT {_COLUMNS} FROM workspace_sqlite_backup_jobs WHERE workspace_id = $1 AND request_key = $2", workspace_id, request_key)
            if existing:
            ...
            if trigger == "scheduled":
                active = await tx.query_raw(f"SELECT {_COLUMNS} FROM workspace_sqlite_backup_jobs WHERE workspace_id = $1 AND trigger = 'scheduled' AND status IN ('pending', 'running') ORDER BY created_at, id LIMIT 1", workspace_id)
                if active:
                    return _payload(active[0])
            counts = await tx.query_raw("SELECT count(*) FILTER (WHERE status IN ('pending', 'running'))::int AS global_count, count(*) FILTER (WHERE workspace_id = $1 AND status IN ('pending', 'running'))::int AS workspace_count FROM workspace_sqlite_backup_jobs", workspace_id)
            ...
            rows = await tx.query_raw(f"""INSERT INTO workspace_sqlite_backup_jobs
                (workspace_id, requested_by_id, trigger, database_names, snapshot_id, snapshot_git_commit_hash, request_key, request_hash)
                VALUES ($1, $2, $3, $4::text[], $5, $6, $7, $8) RETURNING {_COLUMNS}""", workspace_id, requested_by_id, trigger, names, snapshot_id, snapshot_git_commit_hash, request_key, request_hash)
    ...
    async def list_jobs(self, workspace_id: str, *, database_name: str | None = None, snapshot_id: str | None = None, limit: int = 50) -> list[dict[str, Any]]:
        ...
        db = await get_db()
        rows = await db.query_raw(f"SELECT {_COLUMNS} FROM workspace_sqlite_backup_jobs WHERE workspace_id = $1 AND ($2::text IS NULL OR cardinality(database_names) = 0 OR $2 = ANY(database_names)) AND ($3::text IS NULL OR snapshot_id = $3) ORDER BY CASE WHEN status IN ('pending', 'running') THEN 0 ELSE 1 END, created_at DESC, id DESC LIMIT $4", workspace_id, database_name, snapshot_id, limit)
    ...
    async def claim_next(self, owner_token: str) -> dict[str, Any] | None:
        ...
        async with db.tx() as tx:
            await tx.query_raw("SELECT 1 AS locked FROM pg_advisory_xact_lock(hashtextextended($1, 0))", _QUEUE_LOCK)
            claimed = await tx.query_raw(f"""WITH next_job AS (
            ...
            FROM next_job WHERE job.id = next_job.id RETURNING {_qualified_columns('job')}""", owner_token)
    ...
    async def progress(self, job_id: str, owner_token: str, *, completed_databases: int, total_databases: int) -> bool:
        db = await get_db()
        updated = await db.execute_raw("UPDATE workspace_sqlite_backup_jobs SET completed_databases = $1, total_databases = $2, heartbeat_at = NOW(), updated_at = NOW() WHERE id = $3 AND status = 'running' AND owner_token = $4", completed_databases, total_databases, job_id, owner_token)
        return updated == 1

    async def is_cancel_requested(self, job_id: str, owner_token: str) -> bool:
        db = await get_db()
        rows = await db.query_raw("SELECT cancel_requested FROM workspace_sqlite_backup_jobs WHERE id = $1 AND status = 'running' AND owner_token = $2", job_id, owner_token)
    ...
    async def finish(self, job_id: str, owner_token: str, *, status: str, backup_ids: list[str], error_message: str | None = None) -> bool:
        ...
        async with db.tx() as tx:
            await tx.query_raw("SELECT 1 AS locked FROM pg_advisory_xact_lock(hashtextextended($1, 0))", _QUEUE_LOCK)
            rows = await tx.query_raw(f"UPDATE workspace_sqlite_backup_jobs SET status = CASE WHEN cancel_requested THEN 'cancelled' ELSE $1 END, backup_ids = $2::text[], error_message = $3, finished_at = NOW(), updated_at = NOW(), heartbeat_at = NOW() WHERE id = $4 AND status = 'running' AND owner_token = $5 RETURNING {_COLUMNS}", status, backup_ids, error_message, job_id, owner_token)
            if not rows:
                return False
            if str(rows[0].get("status")) == "failed" and str(rows[0].get("trigger")) == "scheduled" and not bool(rows[0].get("cancel_requested")):
                ...
                await tx.query_raw("""INSERT INTO workspace_sqlite_backup_jobs
                    ...
                    ON CONFLICT (workspace_id, request_key) DO NOTHING""", rows[0]["workspace_id"], rows[0].get("requested_by_id"), rows[0].get("database_names") or [], rows[0].get("snapshot_id"), rows[0].get("snapshot_git_commit_hash"), retry_key, rows[0]["request_hash"])
            return True

    async def cancel(self, workspace_id: str, job_id: str) -> dict[str, Any] | None:
        db = await get_db()
        async with db.tx() as tx:
            rows = await tx.query_raw(f"UPDATE workspace_sqlite_backup_jobs SET status = CASE WHEN status = 'pending' THEN 'cancelled' ELSE status END, cancel_requested = CASE WHEN status = 'running' THEN TRUE ELSE FALSE END, finished_at = CASE WHEN status = 'pending' THEN NOW() ELSE finished_at END, updated_at = NOW() WHERE workspace_id = $1 AND id = $2 AND status IN ('pending', 'running') RETURNING {_COLUMNS}", workspace_id, job_id)
            if rows:
                return _payload(rows[0])
    ...
    async def stale_running(self, *, older_than_seconds: int = 120, limit: int = 50) -> list[dict[str, Any]]:
        db = await get_db()
        rows = await db.query_raw(f"SELECT {_COLUMNS} FROM workspace_sqlite_backup_jobs WHERE status = 'running' AND COALESCE(heartbeat_at, started_at, created_at) < NOW() - ($1 * INTERVAL '1 second') ORDER BY COALESCE(heartbeat_at, started_at, created_at) LIMIT $2", older_than_seconds, max(1, min(limit, 50)))
        return [_payload(row) for row in rows]

    async def interrupt(self, job_id: str, owner_token: str, *, error_message: str) -> bool:
        db = await get_db()
        updated = await db.execute_raw("UPDATE workspace_sqlite_backup_jobs SET status = 'interrupted', error_message = $1, finished_at = NOW(), updated_at = NOW() WHERE id = $2 AND status = 'running' AND owner_token = $3", error_message, job_id, owner_token)
        return updated == 1

    async def prune_terminal(self, *, older_than_days: int = 30, limit: int = 100) -> list[str]:
        db = await get_db()
        rows = await db.query_raw("DELETE FROM workspace_sqlite_backup_jobs WHERE id IN (SELECT id FROM workspace_sqlite_backup_jobs WHERE status IN ('completed', 'failed', 'cancelled', 'interrupted') AND finished_at < NOW() - ($1 * INTERVAL '1 day') ORDER BY finished_at LIMIT $2) RETURNING id", older_than_days, max(1, min(limit, 100)))
        return [str(row["id"]) for row in rows]

    async def _owned_update(self, job_id: str, owner_token: str, assignments: str) -> bool:
        db = await get_db()
        updated = await db.execute_raw(f"UPDATE workspace_sqlite_backup_jobs SET {assignments} WHERE id = $1 AND status = 'running' AND owner_token = $2", job_id, owner_token)
        return updated == 1
