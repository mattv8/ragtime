...
function applyFilterCondition(row: Record<string, unknown>, condition: FilterCondition): boolean {
  ...
  switch (condition.operator) {
    ...
    case 'not_equals':
      return strVal !== fv;
    case 'contains':
      return strVal.includes(fv);
    case 'not_contains':
      return !strVal.includes(fv);
    case 'gt': {
      const n = Number(value);
      const f = Number(condition.value);
      return Number.isFinite(n) && Number.isFinite(f) ? n > f : String(value) > condition.value;
    }
    case 'gte': {
      const n = Number(value);
      const f = Number(condition.value);
...
function buildSqlFromFilters(tableName: string, filters: FilterCondition[]): string {
  ...
  const active = filters.filter(
    (f) =>
      f.column &&
      (FILTER_OPERATORS.find((o) => o.value === f.operator)?.needsValue === false ||
        f.value.trim()),
  );
...
export function WorkspaceSqliteInspectorModal({
  ...
  useEffect(() => {
    ...
    const events = new EventSource(
      `/indexes/userspace/workspaces/${encodeURIComponent(workspaceId)}/sqlite/databases/events`,
    );
  ...
  const handleTableImportFile = useCallback(
    async (file: File | null) => {
      ...
      try {
        const result = await api.importUserSpaceSqliteTable(
          workspaceId,
          selectedDatabase.name,
  ...
  const handleSaveNewRow = useCallback(async () => {
    ...
    try {
      ...
      const result = await api.insertUserSpaceSqliteRow(
        workspaceId,
        selectedDatabase.name,
        selectedTableName,
        { values },
  ...
  const handleUpdateCell = useCallback(
    async (originalRow: Record<string, unknown>, colName: string, newValue: unknown) => {
      ...
      try {
        ...
        const result = await api.updateUserSpaceSqliteRow(
          workspaceId,
          selectedDatabase.name,
          selectedTableName,
          {
  ...
  const handleDeleteRow = useCallback(
    async (row: Record<string, unknown>) => {
      ...
      try {
        ...
        const result = await api.deleteUserSpaceSqliteRow(
          workspaceId,
          selectedDatabase.name,
          selectedTableName,
          { row_key: rowKey },
  ...
  const handleRenameColumn = useCallback(
    async (columnName: string, newColumnName: string) => {
      ...
      try {
        const alterations: SqliteInspectorAlterationStep[] = [
          { op: 'rename_column', column_name: columnName, new_column_name: newColumnName },
        ];
        const result = await api.alterUserSpaceSqliteTable(
          workspaceId,
          selectedDatabase.name,
        ...
        <div className="modal-header">
          <div className="userspace-sqlite-title">
            ...
            <h3>
              SQLite Inspector
              {workspaceName ? (
                <span className="userspace-sqlite-workspace-name">{workspaceName}</span>
              ) : null}
            </h3>
          </div>
      ...
      {showCreateForm && (
        <div className="userspace-sqlite-form">
              ...
              {newTableColumns.map((col, idx) => (
                <div key={idx} className="userspace-sqlite-column-row">
                  ...
                  <select
                    ...
                    value={col.type}
                    onChange={(e) =>
                      updateColumn(idx, { type: e.target.value as SqliteInspectorColumnType })
                    }
                  >
                    {COLUMN_TYPES.map((t) => (
          ...
          <div className="userspace-sqlite-form-actions">
            ...
            >
              {creating ? <Loader2 size={12} className="spinning" /> : <Save size={12} />} Create
              table
            </button>
          </div>
        </div>
...
interface TableStepProps {
  ...
  onChangeColumnType: (
    column: SqliteInspectorColumnInfo,
    newType: SqliteInspectorColumnType,
  ) => void;
      ...
      <div className="userspace-sqlite-step-header">
        <div>
          ...
          <div className="userspace-sqlite-meta">
            ...
            <span>
              {rowPage?.total ?? 0} row{(rowPage?.total ?? 0) === 1 ? '' : 's'}
            </span>
          </div>
        </div>
        <div className="wizard-tabs wizard-tabs--compact">
...
function RowsPane({
  ...
  const rows = useMemo(() => rowPage?.rows ?? [], [rowPage]);
  ...
  const allSelected =
    filteredRows.length > 0 && filteredRows.every((_, idx) => selectedRows.has(idx));
      ...
      <div className="userspace-sqlite-rows-toolbar">
        <div className="userspace-sqlite-rows-toolbar-left">
          ...
          {canEdit && schema && (
            <button
              type="button"
              className="btn btn-primary btn-sm"
              onClick={onShowAddRowForm}
              disabled={showAddRowForm}
            >
        ...
        <div className="userspace-sqlite-rows-toolbar-right">
          ...
          >
            <Filter size={14} />
            {filters.length > 0 && (
              <span className="userspace-sqlite-filter-badge">{filters.length}</span>
            )}
          </button>
          <button
            type="button"
        ...
        <div className="userspace-sqlite-filter-bar">
            ...
            filters.map((filter, idx) => {
              ...
              return (
                <div key={filter.id} className="userspace-sqlite-filter-row">
                  ...
                  <span className="userspace-sqlite-filter-keyword">
                    {idx === 0 ? 'where' : 'and'}
                  </span>
                  <select
          ...
          <div className="userspace-sqlite-filter-actions">
            ...
            <button
              type="button"
              className="btn btn-secondary btn-sm"
              onClick={() => onOpenInSql(buildSqlFromFilters(tableName, filters))}
            >
            ...
            {schema.columns.map((col) => (
              <div key={col.name} className="userspace-sqlite-row-form-field">
                <label className="field-label">
                  {col.name}
                  <span className="userspace-sqlite-col-type">
                    {' '}
                    ({col.type || 'ANY'}
                    {col.primary_key ? ', PK' : ''}
                    {col.not_null ? ', NN' : ''})
                ...
                <input
                  ...
                  onChange={(e) =>
                    onNewRowValuesChange({ ...newRowValues, [col.name]: e.target.value })
                  }
                  placeholder={
                    col.primary_key && col.type.toUpperCase().includes('INT')
                      ? 'auto'
                      : 'value or NULL'
                  }
                />
          ...
          <table className="userspace-sqlite-table">
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                <tr>
                  <td
                    colSpan={columns.length + (canEdit ? 2 : 0)}
                    className="userspace-sqlite-empty-row"
                  >
                    {rows.length > 0
                ...
                filteredRows.map((row, idx) => (
                  ...
                  >
                    {canEdit && (
                      <td
                        className="userspace-sqlite-checkbox-col"
                    ...
                    {columns.map((col) => {
                      ...
                      const colType =
                        schema?.columns.find((c) => c.name === col.name)?.type ?? 'TEXT';
                      return (
                        <td
                          key={col.name}
                        ...
                        >
                          {isThisCell ? (
                              ...
                              onBlur={() => {
                                ...
                                if (cell && cell.draft !== cell.original) {
                                  void onUpdateCell(
                                    row,
                                    col.name,
                                    parseCellInput(cell.draft, colType),
                                  );
                                }
                              }}
                              onKeyDown={(event) => {
                            ...
                            <span
                              className={
                                row[col.name] === null ? 'userspace-sqlite-cell-null' : undefined
                              }
                            >
                              {renderCellValue(row[col.name])}
                            </span>
                    ...
                    {canEdit && (
                      <td
                        className="userspace-sqlite-actions-col"
                        onClick={(event) => event.stopPropagation()}
                      >
                        <DeleteConfirmButton
...
function SchemaPane({
  ...
  const [draftColumn, setDraftColumn] = useState<SqliteInspectorColumnSpec>({
    name: '',
    type: 'TEXT',
  });
    ...
    <div className="userspace-sqlite-schema-pane">
        ...
        <div className="userspace-sqlite-form">
          <div className="userspace-sqlite-column-row">
            ...
            <select
              ...
              onChange={(e) =>
                setDraftColumn({
                  ...draftColumn,
                  type: e.target.value as SqliteInspectorColumnType,
                })
              }
            >
              {COLUMN_TYPES.map((t) => (
          ...
          <div className="userspace-sqlite-form-actions">
            ...
            <button
              ...
              disabled={submitting || !draftColumn.name.trim()}
            >
          ...
          <tbody>
            {schema.columns.map((col) => (
              <tr key={col.name}>
                <td
                  className={
                    canEdit && !col.primary_key ? 'userspace-sqlite-editable-cell' : undefined
                  }
                  onClick={() => {
                ...
                <td
                  className={
                    canEdit && rowCount === 0 && !col.primary_key
                      ? 'userspace-sqlite-editable-cell'
                      : undefined
                  }
                  onClick={() => {
                    if (
                ...
                >
                  {canEdit && editingTypeColumn === col.name ? (
                    <select
                      ...
                      autoFocus
                      value={
                        (COLUMN_TYPES.includes(col.type as SqliteInspectorColumnType)
                          ? col.type
                          : 'TEXT') as SqliteInspectorColumnType
                      }
                      onClick={(event) => event.stopPropagation()}
                      onChange={(event) => {
                    ...
                    <span
                      title={
                        rowCount > 0 ? 'Empty the table before changing column type' : undefined
                      }
                    >
                      {col.type || '—'}
                    </span>
                ...
                <td>
                  {col.default_value ?? <span className="userspace-sqlite-cell-null">NULL</span>}
                </td>
                {canEdit && (
      ...
      {schema.sql && (
        <details className="userspace-sqlite-sql-details">
          ...
          <pre className="userspace-sqlite-sql">
    ...
    <div className="userspace-sqlite-console-pane">
      <div className="userspace-sqlite-console-editor-card">
        ...
        <div className="userspace-sqlite-console-toolbar">
          <span className="userspace-sqlite-console-hint">
            Read-only ·{' '}
            {result
