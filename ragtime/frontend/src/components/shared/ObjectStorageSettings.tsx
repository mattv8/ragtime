...
export function ObjectStorageSettings(): JSX.Element {
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage(e instanceof Error ? e.message : 'Unable to save object storage settings');
    } finally {
      setBusy(null);
    }
  };
  ...
  return (
    ...
    >
      ...
      <p className="field-help">
        Local storage is ready with no setup. External credentials are never displayed; leave secret
        fields empty to preserve saved values.
      </p>
      ...
      <div className="form-group">
        ...
        <button
          ...
          disabled={busy !== null}
        >
          {busy === 'save' ? <Loader2 size={14} className="spinning" /> : null} Save storage
          settings
        </button>
      </div>
