...
export function AgentAccessModal({
    ...
    <>
      <div className="userspace-agent-access-intro">
        ...
        <small className="userspace-agent-access-note userspace-muted" role="note">
          Allow this workspace&apos;s agent to use file/runtime tools in another workspace you can
          access. A SQLite grant covers all data in app.sqlite3, including future data. Only the
          target workspace owner can change it.
        </small>
      </div>
          ...
          <div className="userspace-members-list">
            {grants.length === 0 ? (
              ...
              grants.map((grant) => {
                ...
                const draft = draftsByTargetId[grant.target_workspace_id] ?? buildGrantDraft(grant);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="userspace-agent-access-control-row">
                      ...
                      >
                        ...
                        <button
                          ...
                          disabled={isOperationLocked || rowOperationActive || !canGrantReadWrite}
                        >
                          Read / Write
                        </button>
                      </div>
                    ...
                    <div className="userspace-agent-access-control-row">
                      <span className="userspace-agent-access-control-label">Shared SQLite</span>
                      <div
                        className="userspace-member-role-toggle userspace-agent-access-toggle"
                        role="group"
                      ...
                      >
                        {(['none', 'read', 'read_write'] as const).map((mode) => (
                          <button
                            ...
                            disabled={isOperationLocked || rowOperationActive || !canGrantSqlite}
                          >
  ...
  const tabs: ModalTab[] = [
    { id: 'workspace-grants', label: 'Workspace Grants', content: workspaceGrantsPanel },
    ...(agentCollaborationSection
      ? [
          {
            id: 'agent-collaboration',
            label: 'Agent Collaboration',
            content: agentCollaborationSection,
          },
        ]
      : []),
    ...(connectAgentSection
      ? [{ id: 'coding-agent-setup', label: 'Coding Agent Setup', content: connectAgentSection }]
      : []),
  ];
    ...
    <div className="modal-overlay" onClick={handleClose}>
      ...
      >
        ...
        <div className="modal-body">
          {hasTabs ? (
            <ModalTabs
              idPrefix="userspace-agent-access"
              label="Agent Access"
              tabs={tabs}
