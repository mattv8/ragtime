...
const X_ICON_SVG = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg>';
...
export const RichChatInput = forwardRef<RichChatInputHandle, RichChatInputProps>(function RichChatInput(
  {
    segments,
    onChange,
    onSubmit,
  ...
  const setEditorEl = useCallback((node: HTMLDivElement | null) => {
  ...
  useEffect(() => { onRemoveReferenceRef.current = onRemoveReference; }, [onRemoveReference]);
  useEffect(() => { onOpenReferenceRef.current = onOpenReference; }, [onOpenReference]);
  ...
  const renderSegments = useCallback((next: RichChatSegment[]) => {
    const editor = editorRef.current;
    if (!editor) return;
    ...
    if (savedCaretIndexRef.current !== null) {
      savedCaretIndexRef.current = Math.min(savedCaretIndexRef.current, editor.childNodes.length);
    }
  }, [buildChipElement]);
    ...
    editor.childNodes.forEach((node) => {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (refId) {
          ...
          if (payload) {
            try {
              result.push({ type: 'ref', reference: JSON.parse(payload) as ChatContextReference });
              return;
            } catch {
  ...
  const insertReferenceAtCaret = useCallback((reference: ChatContextReference) => {
    const editor = editorRef.current;
    if (!editor) return;

    const selection = window.getSelection();
    const liveInEditor = !!selection
...
export { formatReferenceLabel as formatContextReferenceLabel, formatReferenceLocation as formatContextReferenceLocation };
