...
function readStoredWorkspaceScmWizardActivity(
  workspaceId: string,
): WorkspaceScmWizardActivity | null {
  try {
...
function setWorkspaceScmWizardActivity(
  workspaceId: string,
  activity: WorkspaceScmWizardActivity | null,
): void {
  if (activity) {
    ...
    if (activity.kind === 'preview' && activity.status === 'ready') {
      try {
        window.sessionStorage.setItem(
          workspaceScmWizardActivityStorageKey(workspaceId),
...
export function useWorkspaceScmWizardActivity(
  workspace: UserSpaceWorkspace | null | undefined,
): WorkspaceScmWizardActivity | null {
  ...
  return useMemo(() => {
    ...
    if (
      workspace.scm_import_task_id &&
      isScmImportPhaseInProgress(workspace.scm_import_task_phase)
    ) {
      return {
        kind: 'import-task',
    ...
    if (localActivity) {
      if (localActivity.kind === 'import-task') {
        if (
          workspace.scm_import_task_id === localActivity.taskId &&
          !isScmImportPhaseInProgress(workspace.scm_import_task_phase)
        ) {
    ...
    return null;
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentionally depends on specific workspace fields, not the whole workspace object
...
function syncIntervalToSlider(seconds: number): number {
  ...
  return Math.max(
    0,
    Math.min(100, (Math.log(seconds / SCM_SYNC_INTERVAL_MIN) / SCM_SYNC_INTERVAL_SCALE) * 100),
  );
}
...
function isArchiveExportPhaseInProgress(
  phase: UserSpaceWorkspaceArchiveExportTask['phase'] | null | undefined,
): boolean {
  return Boolean(phase && !isArchiveExportTaskTerminal(phase));
}

function isArchiveImportPhaseInProgress(
  phase: UserSpaceWorkspaceArchiveImportTask['phase'] | null | undefined,
): boolean {
...
function getArchiveImportProgress(
  task: UserSpaceWorkspaceArchiveImportTask,
): { currentStep: number; totalSteps: number; percent: number } | null {
...
function isScmImportPhaseInProgress(
  phase: UserSpaceWorkspaceScmImportTaskPhase | null | undefined,
): boolean {
  return Boolean(
    phase && ['queued', 'previewing', 'cloning', 'importing', 'backfilling'].includes(phase),
  );
}
...
function getScmImportTaskProgressPercent(
...
function buildArchiveExportTaskPlaceholder(
  workspace: UserSpaceWorkspace,
): UserSpaceWorkspaceArchiveExportTask | null {
...
function buildArchiveImportTaskPlaceholder(
  workspace: UserSpaceWorkspace,
): UserSpaceWorkspaceArchiveImportTask | null {
...
function buildScmImportTaskPlaceholder(
  workspace: UserSpaceWorkspace,
): UserSpaceWorkspaceScmImportTask | null {
...
export function WorkspaceScmWizard({
  workspace,
  onClose,
  onSyncComplete,
  onAskAgent,
  onWorkspaceChanged,
}: WorkspaceScmWizardProps) {
  const initialScm = workspace.scm;
  const scmWizardActivity = useWorkspaceScmWizardActivity(workspace);
  const hasActiveArchiveExportTask =
    Boolean(workspace.archive_export_task_id) &&
    isArchiveExportPhaseInProgress(workspace.archive_export_task_phase);
  const hasActiveArchiveImportTask =
    Boolean(workspace.archive_import_task_id) &&
    isArchiveImportPhaseInProgress(workspace.archive_import_task_phase);
  ...
  const [activeTab, setActiveTab] = useState<ModalTab>(
    hasActiveScmImportTask ? 'git-source' : 'archive',
  );
  ...
  const [repoVisibility, setRepoVisibility] = useState<RepoVisibilityResponse['visibility'] | null>(
    initialScm?.repo_visibility || null,
  );
  ...
  const [sqlImportResult, setSqlImportResult] = useState<UserSpaceWorkspaceSqliteImportTask | null>(
    null,
  );
  ...
  const [archiveStep, setArchiveStep] = useState<ArchiveStep>(
    shouldOpenArchiveByDefault ? 'configure' : 'choose',
  );
  ...
  const [archiveImportTask, setArchiveImportTask] =
    useState<UserSpaceWorkspaceArchiveImportTask | null>(null);
  const [scmImportTask, setScmImportTask] = useState<UserSpaceWorkspaceScmImportTask | null>(() =>
    buildScmImportTaskPlaceholder(workspace),
  );
  ...
  const [autoPushEnabled, setAutoPushEnabled] = useState(
    initialScm?.auto_sync_policy === 'auto_push',
  );
  const [autoPullEnabled, setAutoPullEnabled] = useState(Boolean(initialScm?.auto_pull_enabled));
  const [autoPushIntervalSeconds, setAutoPushIntervalSeconds] = useState(
    initialScm?.auto_push_interval_seconds ?? 3600,
  );
  const [autoPullIntervalSeconds, setAutoPullIntervalSeconds] = useState(
    initialScm?.auto_pull_interval_seconds ?? 3600,
  );
  const [autoPushStartMinute, setAutoPushStartMinute] = useState<number | null>(
    initialScm?.auto_push_start_minute ?? null,
  );
  const [autoPushTimezone, setAutoPushTimezone] = useState<string | null>(
    initialScm?.auto_push_timezone ?? null,
  );
  const [autoPullStartMinute, setAutoPullStartMinute] = useState<number | null>(
    initialScm?.auto_pull_start_minute ?? null,
  );
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((settings) => {
        if (cancelled) return;
        setSqlImportMaxBytes(
          settings.userspace_sqlite_import_max_bytes || SQLITE_IMPORT_DEFAULT_MAX_BYTES,
        );
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- effect intentionally scoped; workspace is read via closure and must not retrigger it
  }, [
    archiveExportTask,
    workspace.archive_export_task_id,
    workspace.archive_export_task_phase,
    workspace.id,
    workspace.name,
  ...
  useEffect(() => {
    ...
    archiveImportTask,
    workspace.archive_import_task_id,
    workspace.archive_import_task_phase,
    workspace.id,
    workspace.name,
    workspace.updated_at,
  ]);
  ...
  useEffect(() => {
    ...
    scmImportTask,
    workspace.scm_import_task_id,
    workspace.scm_import_task_phase,
    workspace.id,
    workspace.name,
    workspace.updated_at,
  ]);
  ...
  useEffect(() => {
    ...
    if (
      scmWizardActivity?.kind === 'preview' &&
      scmWizardActivity.status === 'ready' &&
      scmWizardActivity.preview
    ) {
  ...
  const shouldShowStatus = useMemo(() => {
    ...
    if (activeTab === 'git-source' && step === 'result' && scmImportTask && status.type !== 'error')
      return false;
  ...
  useEffect(() => {
    ...
    activeScm?.auto_pull_interval_seconds,
    activeScm?.auto_pull_start_minute,
    activeScm?.auto_pull_timezone,
    activeScm?.auto_push_interval_seconds,
    activeScm?.auto_push_start_minute,
    activeScm?.auto_push_timezone,
    activeScm?.auto_sync_policy,
  ]);

  const hasPendingPatToken = useMemo(
    () => hasConfiguredRemote && gitToken.trim().length > 0,
    [gitToken, hasConfiguredRemote],
  );

  const connectedBranch = (activeScm?.git_branch || '').trim();
  const selectedBranchDiffers =
    hasConfiguredRemote &&
  ...
  const hasRunningGitSourceTask = useMemo(() => {
    if (scmWizardActivity?.kind === 'preview' && scmWizardActivity.status === 'running')
      return true;
  ...
  const hasDirtyUpstreamSyncSettings = useMemo(() => {
    ...
    if (
      autoPushEnabled &&
      (autoPushStartMinute !== savedAutoPushStartMinute ||
        autoPushTimezone !== savedAutoPushTimezone)
    ) {
      return true;
    }
    ...
    if (
      autoPullEnabled &&
      (autoPullStartMinute !== savedAutoPullStartMinute ||
        autoPullTimezone !== savedAutoPullTimezone)
    ) {
      return true;
    }
    return false;
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : 'Fields cleared, but failed to clear stored token.',
      );
    } finally {
      setIsLoading(false);
      setLoadingAction(null);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        setStatus({
          type: 'error',
          message: error instanceof Error ? error.message : 'Failed to inspect repository',
        });
      }
    }, 400);
  ...
  async function handlePreview(
    explicitDirection?: 'import' | 'export',
    ...
    const action = options?.forceOverwrite
      ? 'overwrite'
      : explicitDirection === 'export'
        ? 'push'
        : explicitDirection === 'import'
          ? 'pull'
          : hasConfiguredRemote
            ? 'sync'
            : 'preview';
    ...
    try {
      ...
      const requestedDirection =
        explicitDirection ??
        (hasConfiguredRemote ? 'import' : mode === 'export' ? 'export' : 'import');
  ...
  async function handleExecute(): Promise<void> {
    ...
    try {
      ...
      if (direction === 'import') {
        const task = await api.queueUserSpaceWorkspaceScmImport(
          workspace.id,
          payload as UserSpaceWorkspaceScmImportRequest,
        );
        lastNotifiedScmImportTaskIdRef.current = null;
        setWorkspaceScmWizardActivity(workspace.id, {
          kind: 'import-task',
        ...
        const nextResult = await api.exportUserSpaceWorkspaceToScm(
          workspace.id,
          payload as UserSpaceWorkspaceScmExportRequest,
        );
        setWorkspaceScmWizardActivity(workspace.id, null);
  ...
  async function handleSqlImport(): Promise<void> {
    ...
    try {
      ...
      setStatus({
        type: 'error',
        message: error instanceof Error ? error.message : 'SQL import failed.',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const loadArchiveExports = useCallback(
  ...
  useEffect(() => {
    ...
    const pollTask = async () => {
      ...
      pollInFlight = true;
      try {
        const nextTask = await api.getUserSpaceWorkspaceArchiveExportTask(
          archiveExportTask.task_id,
        );
        if (cancelled) return;
        setArchiveExportTask(nextTask);
        if (
          ...
          toast.success(
            nextTask.warnings.length > 0
              ? 'Archive export completed with warnings.'
  ...
  useEffect(() => {
    ...
    const pollTask = async () => {
      ...
      pollInFlight = true;
      try {
        const nextTask = await api.getUserSpaceWorkspaceArchiveImportTask(
          archiveImportTask.task_id,
        );
        if (cancelled) return;
        setArchiveImportTask(nextTask);
        if (
          ...
          toast.success(
            nextTask.warnings.length > 0
              ? 'Archive import completed with warnings.'
  ...
  const activeSqlImportTaskId =
    sqlImportResult && !isSqliteImportTaskTerminal(sqlImportResult.phase)
  ...
  useEffect(() => {
    ...
    async function pollTask(): Promise<void> {
      ...
      try {
        ...
        if (nextTask.phase === 'completed') {
          ...
          setStatus({
            type: 'error',
            message: nextTask.error || nextTask.message || 'SQL import failed.',
          });
        }
      } catch (error) {
        if (!cancelled) {
          setStatus({
            type: 'error',
            message: error instanceof Error ? error.message : 'SQL import status unavailable.',
          });
        }
      } finally {
        pollInFlight = false;
    ...
    intervalId = window.setInterval(() => {
      void pollTask();
    }, SQLITE_IMPORT_POLL_INTERVAL_MS);
    return () => {
      cancelled = true;
      if (intervalId !== null) window.clearInterval(intervalId);
    };
        ...
        <div className="modal-header" style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
          <div style={{ display: 'flex', gap: 0, flex: 1 }}>
            ...
            <button
              ...
              title={
                sqliteEnabled
                  ? 'Import a SQL dump into the workspace SQLite database'
                  : 'Enable SQLite persistence mode on this workspace first'
              }
            >
              SQL Import
            </button>
        ...
        <div className="modal-body" style={{ display: 'grid', gap: 16 }}>
          {activeTab === 'git-source' && !hasConfiguredRemote && (
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                className={`btn btn-sm ${mode === 'export' ? 'btn-primary' : 'btn-secondary'}`}
                onClick={() => {
                  setMode('export');
                  setStep('input');
                }}
          ...
          {activeTab === 'archive' && archiveStep === 'choose' && (
            <div style={{ display: 'grid', gap: 20 }}>
              <p className="userspace-muted" style={{ fontSize: 13, margin: 0 }}>
                Export packages this workspace's files and metadata as a portable archive. Import
                replaces this workspace with an archive exported from another server.
              </p>
              ...
              {archiveMode === 'export' && (
                <div style={{ display: 'grid', gap: 14 }}>
                  <div style={{ display: 'grid', gap: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        className={`btn btn-sm ${archiveFormat === 'zip' ? 'btn-primary' : 'btn-secondary'}`}
                      ...
                      <button
                        className={`btn btn-sm ${archiveFormat === 'tar.gz' ? 'btn-primary' : 'btn-secondary'}`}
                  ...
                  >
                    ...
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      <input
                        type="checkbox"
                        style={{ marginTop: 2 }}
                        checked={archiveIncludeSnapshots}
                        onChange={(event) => setArchiveIncludeSnapshots(event.target.checked)}
                        disabled={isLoading}
                      />
                      <div>
                        <div style={{ fontWeight: 500 }}>Snapshot history</div>
                        <div className="userspace-muted" style={{ fontSize: 12, marginTop: 2 }}>
                          Included only when the workspace has a clean, committed snapshot timeline
                        </div>
                      </div>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      <input
                        type="checkbox"
                        style={{ marginTop: 2 }}
                        checked={archiveIncludeChatHistory}
                        onChange={(event) => setArchiveIncludeChatHistory(event.target.checked)}
                        disabled={isLoading}
                      />
                  ...
                  <p className="userspace-muted" style={{ fontSize: 12, margin: 0 }}>
                    Secret values are not included. Env vars and mount configuration are carried as
                    placeholders.
                  </p>
                </div>
              )}
                    ...
                    {archiveFile ? (
                      ...
                      <div>
                        ...
                        <div style={{ fontWeight: 500 }}>
                          Drop a workspace archive here or click to browse
                        </div>
                  ...
                  >
                    ...
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 500 }}>Snapshot history</div>
                        <div className="userspace-muted" style={{ fontSize: 12, marginTop: 2 }}>
                          Restored only when the archive contains snapshot data and this workspace
                          has no existing timeline
                        </div>
                      </div>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    ...
                    <>
                      ...
                      >
                        ...
                        <span className="userspace-status-pill userspace-status-pill-info">
                          {formatArchiveTaskPhase(archiveExportTask.phase)}
                        </span>
                        {archiveExportTask.archive_size_bytes ? (
                          <span className="userspace-muted" style={{ fontSize: 12 }}>
                            {formatArchiveSize(archiveExportTask.archive_size_bytes)}
                          </span>
                        ) : null}
                      </div>
                      {!isArchiveExportTaskTerminal(archiveExportTask.phase) &&
                        archiveExportTask.total_files > 0 && (
                          <div style={{ display: 'grid', gap: 4 }}>
                            ...
                            >
                              <span>
                                {archiveExportTask.processed_files} /{' '}
                                {archiveExportTask.total_files} files
                                {archiveExportTask.total_bytes > 0
                                  ? ` · ${formatArchiveSize(archiveExportTask.processed_bytes)} / ${formatArchiveSize(archiveExportTask.total_bytes)}`
                                  : ''}
                              </span>
                              <span>
                                {Math.min(
                                  ...
                                  Math.round(
                                    (archiveExportTask.processed_files /
                                      Math.max(1, archiveExportTask.total_files)) *
                                      100,
                                  ),
                                )}
                                %
                      ...
                      {archiveExportTask.warnings.length > 0 && (
                        <div style={{ display: 'grid', gap: 4 }}>
                          {archiveExportTask.warnings.map((warning) => (
                            <div key={warning} className="userspace-muted" style={{ fontSize: 12 }}>
                              {warning}
                            </div>
                          ))}
                        </div>
                  ...
                  {archiveImportTask && (
                    <>
                      ...
                      >
                        ...
                        <span className="userspace-status-pill userspace-status-pill-info">
                          {formatArchiveTaskPhase(archiveImportTask.phase)}
                        </span>
                        {(archiveImportTask.imported_snapshot_count > 0 ||
                          archiveImportTask.imported_chat_count > 0) && (
                          <span className="userspace-muted" style={{ fontSize: 12 }}>
                            {archiveImportTask.imported_snapshot_count} snapshots,{' '}
                            {archiveImportTask.imported_chat_count} chats
                          </span>
                        )}
                      </div>
                      {(() => {
                ...
                >
                  ...
                  <div style={{ display: 'grid', gap: 6 }}>
                    {archiveExports.map((item) => {
                      const hasDownloaded =
                        downloadedExportTaskIds.has(item.task_id) ||
                        lastDownloadedArchiveTaskIdRef.current === item.task_id;
                      ...
                      return (
                        ...
                        >
                          <div style={{ display: 'grid', gap: 2, minWidth: 0 }}>
                            ...
                            <div
                              className="userspace-muted"
                              style={{ fontSize: 11, display: 'flex', gap: 8, flexWrap: 'wrap' }}
                            >
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {activeScm.sync_paused && (
                          <button
                            ...
                            onClick={async () => {
                              try {
                                const resp = await api.updateUserSpaceWorkspaceScmSettings(
                                  workspace.id,
                                  { clear_sync_paused: true },
                                );
                                setScmState(resp.scm);
                                await onSyncComplete({
                                  workspace_id: workspace.id,
                      ...
                      <div style={{ display: 'grid', gap: 10 }}>
                        ...
                        >
                          {upstreamSyncRows.map((row) => (
                            <button
                              ...
                              title={
                                row.enabled
                                  ? `Disable ${row.label.toLowerCase()}`
                                  : `Enable ${row.label.toLowerCase()}`
                              }
                            ...
                            >
                              ...
                              <span style={{ display: 'grid', gap: 3, minWidth: 0, flex: 1 }}>
                                ...
                                >
                                  ...
                                  <span
                                    className={`userspace-status-pill ${row.enabled ? 'userspace-status-pill-success' : 'userspace-status-pill-muted'}`}
                                    style={{ fontSize: 11 }}
                                  >
                                    {row.enabled ? 'On' : 'Off'}
                                  </span>
                                  {row.enabled && (
                                    <span
                                      ...
                                      style={{ fontSize: 11 }}
                                    >
                                      {hasDirtyUpstreamSyncSettings
                                        ? 'Unsaved'
                                        : `Every ${formatSyncInterval(row.interval)}`}
                                    </span>
                                  )}
                                </span>
                                <span className="userspace-muted" style={{ fontSize: 12 }}>
                                  {row.enabled
                                    ? `Runs every ${formatSyncInterval(row.interval)}.`
                                    : row.description}
                                </span>
                              </span>
                            </button>
                        ...
                        {enabledUpstreamSyncCount > 0 && (
                          <div
                            style={{
                              display: 'grid',
                              gridTemplateColumns: useSingleEnabledSyncLayout
                                ? '1fr'
                  ...
                  {(tokenRequired || !storedTokenValid || mode === 'export') && (
                    <label className="form-group" style={{ marginBottom: 0 }}>
                      <input
                        ...
                        placeholder={
                          mode === 'export'
                            ? 'Required for push or repo creation'
              ...
              {mode === 'export' && !hasConfiguredRemote && (
                ...
                >
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input
                      type="checkbox"
                      checked={createRepoIfMissing}
                      onChange={(event) => setCreateRepoIfMissing(event.target.checked)}
                      disabled={isLoading}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input
                      ...
                      onChange={(event) => setCreateRepoPrivate(event.target.checked)}
                      disabled={isLoading}
                    />
          ...
          {activeTab === 'sql-import' && step === 'input' && mode === 'sql-import' && (
            <div style={{ display: 'grid', gap: 14 }}>
              <div className="userspace-muted" style={{ fontSize: 12 }}>
                Upload a PostgreSQL (<code>pg_dump --format=plain</code>), MySQL (
                <code>mysqldump</code>), or generic SQL text dump. It will be converted and imported
                into the workspace SQLite database at <code>.ragtime/db/app.sqlite3</code>.
              </div>
              ...
              >
                ...
                {sqlFile ? (
                  ...
                  <div>
                    ...
                    <div className="userspace-muted" style={{ fontSize: 12, marginTop: 4 }}>
                      Accepts .sql, .dump, .pg, .pgsql, .mysql (plain SQL or PostgreSQL custom dump,
                      max {formatBytes(sqlImportMaxBytes)}
                      {sqlImportLimitLoaded ? '' : ' default'})
                    </div>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                {isSqliteImportTaskTerminal(sqlImportResult.phase) && (
                  ...
                  >
                    ...
                    <div>
                      <strong>
                        {sqlImportResult.message || formatSqliteImportPhase(sqlImportResult.phase)}
                      </strong>
                    </div>
                  </div>
                )}
                  ...
                  >
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: `${getSqliteImportProgressPercent(sqlImportResult)}%`,
                          background: 'var(--color-accent)',
                ...
                {sqlImportResult.warnings.length > 0 && (
                  ...
                  >
                    <strong style={{ fontSize: 13 }}>
                      Warnings ({sqlImportResult.warnings.length})
                    </strong>
                    <div style={{ maxHeight: 160, overflowY: 'auto', marginTop: 8 }}>
                      {sqlImportResult.warnings.map((warning, index) => (
                        <div
                          key={index}
                          style={{ fontFamily: 'var(--font-mono)', fontSize: 11, padding: '2px 0' }}
                        >
                          {warning}
                        </div>
                      ))}
                ...
                {sqlImportResult.errors.length > 0 && (
                  ...
                  >
                    <strong style={{ fontSize: 13 }}>
                      Errors ({sqlImportResult.errors.length})
                    </strong>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  ...
                  <span
                    className={`userspace-status-pill ${preview.direction === 'import' ? 'userspace-status-pill-info' : 'userspace-status-pill-success'}`}
                  >
                ...
                <div className="userspace-muted" style={{ fontSize: 12, marginTop: 8 }}>
                  Workspace HEAD:{' '}
                  {preview.workspace_head_commit_hash || preview.local_commit_hash || 'none'}
                </div>
                <div className="userspace-muted" style={{ fontSize: 12 }}>
                  Remote HEAD:{' '}
                  {preview.remote_head_commit_hash || preview.remote_commit_hash || 'none'}
                </div>
                <div className="userspace-muted" style={{ fontSize: 12 }}>
              ...
              {(preview.will_overwrite_local || preview.will_overwrite_remote) && (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="userspace-muted" style={{ fontSize: 12, marginTop: 4 }}>
                      This action will replace existing{' '}
                      {preview.will_overwrite_local ? 'workspace' : 'remote'} state. Review the
                      sample paths below before continuing.
                    </div>
                  </div>
                </div>
              )}
                ...
                >
                  ...
                  <div style={{ display: 'grid', gap: 4, maxHeight: 240, overflowY: 'auto' }}>
                    {preview.changed_files_sample.map((path) => (
                      <div key={path} style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                        {path}
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'git-source' &&
            ...
            mode !== 'sql-import' &&
            scmWizardActivity?.kind === 'preview' &&
            scmWizardActivity.status === 'running' && (
              <div style={{ display: 'grid', gap: 14 }}>
                <div
                  style={{ padding: 12, border: '1px solid var(--color-border)', borderRadius: 8 }}
                >
                  ...
                  <div className="userspace-muted" style={{ fontSize: 12, marginTop: 8 }}>
                    Checking the remote branch and calculating the import plan. You can close this
                    modal and return to this progress from the SCM button.
                  </div>
                </div>
              </div>
            )}
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                ...
                {scmImportTask.phase === 'failed' ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <span>
                        {getScmImportTaskProgressPercent(
                          scmImportTask.phase,
                          scmImportTask.progress,
                        )}
                        %
                      </span>
                    </div>
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: `${getScmImportTaskProgressPercent(scmImportTask.phase, scmImportTask.progress)}%`,
                          background: 'var(--color-accent)',
                          transition: 'width 160ms ease',
                        }}
                      />
                ...
                {scmImportTask.phase === 'completed' && scmImportTask.suggested_setup_prompt && (
                  ...
                  >
                    ...
                    <div className="userspace-muted" style={{ fontSize: 12 }}>
                      This keeps bring-up suggestion-only. The agent will inspect the imported repo
                      first, then repair entrypoint and bootstrap configuration only if needed.
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        className="btn btn-primary btn-sm"
                        onClick={() => void handleAskAgentFromScmTask()}
                        disabled={!onAskAgent}
                      >
                        <RefreshCw size={14} />
        ...
        >
          <div>
            ...
            {activeTab === 'git-source' &&
              ...
              mode !== 'sql-import' &&
              scmImportTask &&
              isScmImportTaskTerminal(scmImportTask.phase) && (
                <button
                  className="btn btn-secondary"
                  onClick={() => {
                    setWorkspaceScmWizardActivity(workspace.id, null);
            ...
            {activeTab === 'sql-import' && step === 'result' && mode === 'sql-import' && (
              <button
                ...
                disabled={isLoading || isSqliteImportTaskActive(sqlImportResult)}
              >
                Import Another
              </button>
            )}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {activeTab === 'git-source' &&
              ...
              !hasConfiguredRemote && (
                <button
                  className="btn btn-secondary"
                  onClick={() => void handleClearGitSourceFields()}
                  disabled={isLoading || hasRunningGitSourceTask}
                >
                  {loadingAction === 'clear-fields' ? (
            ...
            {activeTab === 'git-source' &&
              ...
              mode !== 'sql-import' &&
              hasConfiguredRemote &&
              hasScmSettingsMutations && (
                <button
                  className="btn btn-primary"
                  onClick={() => void handleSaveScmSettings()}
                  disabled={isLoading || loadingAction === 'save-settings'}
                >
            ...
            {activeTab === 'sql-import' && step === 'input' && mode === 'sql-import' && (
              <button
                ...
                disabled={isLoading || !sqlFile || isSqliteImportTaskActive(sqlImportResult)}
              >
                {isLoading ? (
                  <MiniLoadingSpinner variant="icon" size={14} />
                ) : (
            ...
            {activeTab === 'archive' && archiveStep === 'configure' && archiveMode === 'export' && (
              <>
                {archiveExportTask?.phase === 'completed' &&
                  archiveExportTask.archive_file_name && (
                    <button
                      className="btn btn-secondary"
                ...
                <button
                  className="btn btn-primary"
                  onClick={() => void handleQueueArchiveExport()}
                  disabled={
                    isLoading ||
                    Boolean(
                      archiveExportTask && !isArchiveExportTaskTerminal(archiveExportTask.phase),
                    )
                  }
                >
                  {isLoading ? (
            ...
            {activeTab === 'archive' && archiveStep === 'configure' && archiveMode === 'import' && (
              <button
                ...
                disabled={
                  ...
                  !archiveFile ||
                  Boolean(
                    archiveImportTask && !isArchiveImportTaskTerminal(archiveImportTask.phase),
                  )
                }
              >
                {isLoading ? (
              ...
              activeScm?.remote_role === 'upstream' && (
                <>
                  <button
                    ...
                    title={
                      selectedBranchDiffers
                        ? `Pull branch ${gitBranch.trim()}`
                        : 'Pull from the connected branch'
                    }
                  >
                    {loadingAction === 'pull' ? (
                      <MiniLoadingSpinner variant="icon" size={14} />
                  ...
                  <button
                    className="btn btn-secondary"
                    onClick={() => void handlePreview('export')}
                    disabled={isLoading}
                  >
                    {loadingAction === 'push' ? (
                      <MiniLoadingSpinner variant="icon" size={14} />
                  ...
                  <div style={{ position: 'relative', alignSelf: 'stretch' }}>
                    <button
                      ...
                      disabled={isLoading}
                      title="More options"
                      style={{ padding: '6px 8px', minWidth: 0, height: '100%' }}
                    >
                      &#8230;
                    </button>
                    {showMoreMenu && (
                      ...
                      >
                        ...
                        <Popover
                          ...
                          content={
                            <span style={{ fontSize: 11 }}>
                              Replaces all local files with the remote state. Local-only changes
                              will be lost.
                            </span>
                          }
                        >
                          <DeleteConfirmButton
                            onDelete={() => {
                              setShowMoreMenu(false);
                              void handlePreview('import', { forceOverwrite: true });
                            }}
                            disabled={isLoading}
                            className="btn btn-sm btn-danger"
                            style={{ width: '100%' }}
                        ...
                        <Popover
                          ...
                          content={
                            <span style={{ fontSize: 11 }}>
                              Removes the configured Git remote and clears the stored token.
                            </span>
                          }
                        >
                          <DeleteConfirmButton
              ...
              activeScm?.remote_role !== 'upstream' && (
                <>
                  <button
                    ...
                    title={
                      selectedBranchDiffers
                        ? `Sync branch ${gitBranch.trim()}`
                        : 'Sync with the connected branch'
                    }
                  >
                    {isLoading ? (
                      <MiniLoadingSpinner variant="icon" size={14} />
            ...
            {activeTab === 'git-source' && step === 'review' && preview && (
              <button
                ...
                disabled={
                  isLoading ||
                  (!preview.can_proceed_without_force &&
