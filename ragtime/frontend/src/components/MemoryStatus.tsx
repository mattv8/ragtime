...
export function MemoryStatus({
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent) {
      ...
      if (!statusRef.current?.contains(target) && !dropdownRef.current?.contains(target)) {
        setExpanded(false);
      }
    }
      ...
      >
        ...
        {isLoading && (
          <span className="memory-status-bar">
            <span className="memory-status-bar-fill" style={{ width: `${loadingProgress}%` }} />
          </span>
        )}
      </button>
            ...
            <div className="memory-status-section">
              ...
              <div className="memory-status-row">
                ...
                <span className="memory-status-value">
                  {formatSizeMB(memory.total_mb - memory.available_mb)} /{' '}
                  {formatSizeMB(memory.total_mb)}
                </span>
              </div>
              <div className="memory-status-row">
                <span>Available</span>
            ...
            {(isLoading || sortedDetails.length > 0) && (
              <div className="memory-status-section">
                <h4>
                  ...
                  {!sequential_loading && isLoading && (
                    <span className="memory-status-badge">Parallel</span>
                  )}
                </h4>
  ...
  return (
    <div className={statusClass}>
      ...
      {detail.chunk_count !== null && detail.chunk_count !== undefined && !isLoading && (
        <span className="memory-status-index-chunks">
          {detail.chunk_count.toLocaleString()} chunks
        </span>
      )}
      ...
      {detail.error && (
        <span className="memory-status-index-error" title={detail.error}>
          Error
        </span>
      )}
    </div>
