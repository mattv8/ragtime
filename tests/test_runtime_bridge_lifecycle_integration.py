    ...
    async def test_actual_startup_pipeline_refresh_and_recycle_keep_latest_private_token(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            session = WorkerSession(
                id="worker-1", workspace_id="ws", provider_session_id="provider", workspace_root=root,
                workspace_files_path=files, sandbox_spec=SandboxSpec(workspace_id="ws", workspace_files_path=files, rootfs_path=root / "rootfs"),
                pty_access_token="pty", workspace_env={"RAGTIME_BRIDGE_URL": "http://bridge", "RAGTIME_BRIDGE_TOKEN_FILE": "/run/.ragtime-bridge/token"},
                workspace_env_visibility={}, workspace_mounts=[], mount_targets_to_clear=set(), state="starting", devserver_running=False,
                devserver_port=None, devserver_command=None, launch_framework=None, launch_cwd=None, last_error=None,
                runtime_operation_id="start-1", runtime_operation_phase="queued", runtime_operation_started_at=None,
                runtime_operation_updated_at=None, updated_at=datetime.now(timezone.utc), bridge_credential_mode="worker_file",
                bridge_session_id="session-1", bridge_token_file_initial_token=first,
            )
            ...
            with (
                ...
                refreshed = await service.refresh_bridge_credential(session.id, token=latest, expected_session_id="session-1", expected_revision=0, request_id="refresh-1")
