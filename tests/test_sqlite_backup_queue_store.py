...
class _Tx:
    ...
    async def query_raw(self, query, *params):
        ...
        if "request_key" in query and "SELECT" in query: return self.existing
        if "global_count" in query: return [{"global_count": 0, "workspace_count": 0}]
        if "INSERT" in query: return [self.row]
        return []

    def __init__(self):
        self.calls = []
        self.existing = []
        self.row = {"id": "job", "workspace_id": "ws", "trigger": "manual", "database_names": [], "request_key": "key", "request_hash": "hash", "status": "pending"}


class _Db:
    def __init__(self): self.transaction = _Tx()
...
class SqliteBackupQueueStoreTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_ownership_loss_is_cancelled_but_database_errors_propagate(self):
        class MissingJobDb:
            async def query_raw(self, *_): return []
        ...
        class BrokenDb:
            async def query_raw(self, *_): raise RuntimeError("database unavailable")
