...
class ShareUnlockThemeTests(unittest.TestCase):
    def test_render_injects_default_theme_pack_and_stable_asset(self) -> None:
        ...
        self.assertIn("['default', 'modern', 'serif']", html)
        self.assertIn('"modern"', html)
    ...
    def test_share_theme_entry_uses_shared_token_sources_without_duplicate_palette(self) -> None:
        ...
        template = (Path(__file__).resolve().parents[1] / "ragtime" / "userspace" / "templates" / "share_unlock_prompt.tsx").read_text(encoding="utf-8")

        self.assertIn("@import './fonts.css';", css)
        self.assertIn("@import './theme.css';", css)
        self.assertIn("@import './themes/modern.css';", css)
        ...
        self.assertIn(
            "border-radius: var(--workbench-surface-radius, var(--radius-lg));",
            css,
        )
        self.assertEqual(
            css.count("border-radius: var(--workbench-control-radius, var(--radius-md));"),
            2,
        )
